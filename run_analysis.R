...
##  The directories and files used by this script, as detailed
##  above, must be present in the working directory when this
##  script is run.
...
reqVars <- grepl("mean|std", fdf$V2, ignore.case = TRUE)

##  86 required variables are identified by the search above.
...
##  Select a subset of dfM with only the (88) needed columns.
sub_dfM <- dfM[, reqCols]

##  The large, merged DF (10,299 x 563) is not needed further.
rm(dfM)

##  The needed data is in sub_dfM - 10,299 rows x 88 cols
...
##
## Step 4: Label the df columns with descriptive variable names.  These have
##         been read from the features.txt file into the fdf data frame.
...
##  The tidy Data Set, tidy_df, has 180 rows x 88 cols, with:
##      - One summarized observation per row, grouped by Activity & Subject.
##      - one variable per column -- 2 x Fixed and 86 x Measured variables.
...
##
##         (Please see the CodeBook and README for the justification and details.)
##
...
new_Accdf <- data.frame(ASdf, Device = "Acc",
...
new_Gyrodf <- data.frame(ASdf, Device = "Gyro",
...
##  The columns of new_Gyrodf and new_Accdf correspond exactly.
##
##      - Set the new_Gyrodf colnames to be the same as new_Accdf.
...
##              - Each row has :
##                   - 3 x Fixed variables: Activity, Subject, Device, followed by
